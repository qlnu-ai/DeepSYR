...
class TextClassificationTask(BaseTask):
    ...
    def __init__(self, backend: str, model_name: str, num_labels: int, **kwargs):
        """
        初始化文本分类任务
        
        Args:
            backend: 后端框架类型，目前支持"transformers"和"paddle"
            model_name: 模型名称，如"bert-base-chinese"
            num_labels: 标签数量
            **kwargs: 其他配置参数
        """
        ...
        if backend not in ["transformers", "paddle"]:
            raise ValueError(f"文本分类不支持后端类型: {backend}")
    ...
    def prepare_model(self, **kwargs):
        ...
        """
        # 合并参数
        model_kwargs = {**self.config, **kwargs, "num_labels": self.num_labels}
    ...
    def load(self, load_path: str, **kwargs):
        ...
        if self.backend == "transformers":
            ...
            import paddlenlp
            self.model = paddlenlp.transformers.AutoModelForSequenceClassification.from_pretrained(
                load_path, **model_kwargs
            )
        else:
